...
if (items.length < 1) {
  console.log(`当日无新资讯（${items.length} 条），不出刊，保持前一期。`);
  process.exit(0);
}
...
2. 同一事件多条报道只选一条最权威的。
3. risk 版块注意子题多样性：逾期/资产质量、欺诈骗贷、催收乱象、借款人权益保护、合规处罚都属于 risk——同一期"行政处罚/罚单"类条目最多选 2 条，候选中若有其他风险子题（哪怕分数略低）应优先保留，避免整版都是罚单。
4. summary 只能基于候选条目给出的标题和摘要改写（120字内，关键数字用<b></b>包裹）；信息不足就写短些，严禁编造具体数字、机构名或政策细节。
5. reason 是一句话，站在个贷从业者视角说"为什么值得看"。
6. score 为 7.0-10.0 的一位小数（时效/权威/业务相关/影响面/可操作五维综合）。
7. focus 从入选条目中选最重要的一条（给出其 idx），并写一段80-150字的头条导语（同样不得超出素材信息）。
8. stats 给 4 个数字指标，必须直接取自入选条目摘要中出现过的数字；不足4个就少给。
...
(async () => {
  ...
  if (report.items.length < 1) { console.error("无有效条目，不出刊"); process.exit(0); }
